...
class ProgressBarApp:
    ...
    def start_play(self):
        ...
        speed = float(speed_input) if speed_input else 1.0  # 默认速率为1.0
    ...
    def play_progress(self, speed):
        ...
        def update_progress(start_from_time):  # 添加start_from_time参数
            ...
            if not self.paused:
                ...
                elapsed_time += Decimal(0.1) * Decimal(speed)  # 使用Decimal类型进行精确计算

                self.update_progress_time()

                # 使用固定的时间间隔，例如100毫秒
                self.root.after(100, update_progress, start_from_time)  # 传递start_from_time参数
